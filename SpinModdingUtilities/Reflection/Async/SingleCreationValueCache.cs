...
namespace SMU.Reflection.Async; 
...
/// </remarks>
public sealed class SingleCreationValueCache<TKey, TValue>
{
    private readonly ConcurrentDictionary<TKey, (ManualResetEventSlim wh, TValue val)> dict;

    private static KeyValuePair<TKey, (ManualResetEventSlim, TValue)> ExpandKeyValuePair(KeyValuePair<TKey, TValue> kvp) => new(kvp.Key, (null, kvp.Value));
    private static KeyValuePair<TKey, TValue> CompressKeyValuePair(KeyValuePair<TKey, (ManualResetEventSlim, TValue value)> kvp) => new(kvp.Key, kvp.Value.value);
