...
public static class ReflectionUtil
{
    ...
    public static U InvokeMethod<T, U>(this T obj, string methodName, params object[] args)
    {
    ...
    /// Invokes a method from <typeparamref name="T"/> on an object.
    /// </summary>
    /// <typeparam name="T">the type that the method returns</typeparam>
    /// <param name="obj">the object instance</param>
    /// <param name="methodName">the method's name</param>
    /// <param name="args">the method arguments</param>
    /// <returns>the return value</returns>
    ...
    {
        var dynMethod = obj.GetType().GetMethod(methodName, BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
        if (dynMethod == null) throw new MissingMethodException($"Method {methodName} does not exist", nameof(methodName));
        return (T)dynMethod.Invoke(obj, args);
    }
